...
function init(evt) {
    ...
    document.getElementById("selectedColor").style = `background-color: rgb(${state['seg'][0].col[0][0]},${state['seg'][0].col[0][1]},${state['seg'][0].col[0][2]})`; //wskaünik koloru
...
window.onload = function () {
    ...
    ws = new WebSocket("ws://localhost:8888/ws");   //Tworzenie po≥πczenia

    ws.onopen = function (e) {                      //Po≥πczono pomyúlnie, inicjalizacja parametrÛw na stronie
    ...
    ws.onmessage = function (evt) {                 //odbieranie wiadomoúci
        var data = JSON.parse(evt.data);
        if (data["cmd"] == "state") {
            init(evt);
            return;
        }
        else if (data["cmd"] == "lv")               //dane dotyczπce obecnego stanu ledÛw
        {                  
    ...
    canvas.onmousemove = canvas.onmousedown = function (e) {
        if (e.buttons == 1) {
            ...
            document.getElementById("selectedColor").style = `background-color: rgb(${rgba[0]},${rgba[1]},${rgba[2]})`; //wskaünik koloru
            for (i = 0; i < info["state"]["seg"][0].len; i++) {
    ...
    Rinput.oninput = Ginput.oninput = Binput.oninput = function (e) {
        document.getElementById("selectedColor").style = `background-color: rgb(${Rinput.value},${Ginput.value},${Binput.value})`; //wskaünik koloru
